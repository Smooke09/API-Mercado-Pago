...
class MercadoPagoController {
    ...
    axios({
      ...
      .then(response => {
        ...
        if(paymentOfPagament === "credit_card"){
          let convertString = paymentOfPagament.replace("credit_card", "Cartão de Credito")
          res.status(200).json({
          ...
          let convertString = paymentOfPagament.replace("debit_card", "Cartão de Debito")
          res.status(200).json({
            "message": "Pagamento criado com sucesso",
            "Numero Do Pagamento(id)": idGET,
            "Descrição": getDescription,
  ...
  async showTransaction(req, res) {
    ...
    const urlGet = await `https://api.mercadopago.com/point/integration-api/payment-intents/events?startDate=${dataStart}&endDate=${dataEnd}`;
